...
public class TestPolicyAuditProvider extends ProviderSuper {
    ...
    public void testGetAuditRecords() throws PfModelException {
        ...
        List<PolicyAudit> result = new ArrayList<>(provider.getAuditRecords(5, null, null));
        validateAuditRecords(result, 2);
    }

    @Test
    public void testGetAuditRecordsWithPdpGroup() throws PfModelException {

        AuditFilter auditFilter =
                        AuditFilter.builder().recordNum(5).fromDate(null).toDate(null).pdpGroup(TEST_GROUP).build();

        buildAuditRecords(auditFilter);

        List<PolicyAudit> result = new ArrayList<>(provider.getAuditRecords(5, null, null, TEST_GROUP));
        validateAuditRecords(result, 2);
    }

    @Test
    public void testGetAuditRecordsWithPdpGroupAndPolicy() throws PfModelException {

        AuditFilter auditFilter = AuditFilter.builder().recordNum(5).fromDate(null).toDate(null).pdpGroup(TEST_GROUP)
                        .name(POLICY_A.getName()).version(POLICY_A.getVersion()).build();

        buildAuditRecords(auditFilter);

        List<PolicyAudit> result = new ArrayList<>(provider.getAuditRecords(5, null, null, TEST_GROUP, POLICY_A));
        validateAuditRecords(result, 1);
    }

    @Test
    public void testGetAuditRecordsWithPolicy() throws PfModelException {

        AuditFilter auditFilter = AuditFilter.builder().recordNum(5).fromDate(null).toDate(null)
                        .name(POLICY_A.getName()).version(POLICY_A.getVersion()).build();

        buildAuditRecords(auditFilter);

        List<PolicyAudit> result = new ArrayList<>(provider.getAuditRecords(5, null, null, POLICY_A));
        validateAuditRecords(result, 1);
    }
