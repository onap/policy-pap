...
public class PolicyNotifierTest extends PolicyCommonSupport {
    ...
    public void testLoadPoliciesPolicyModelsProviderFactoryWrapper() throws PfModelException {
        final PdpGroup group1 = makeGroup("my group #1", makeSubGroup("sub #1 A", 2, policy1),
        ...
        List<ToscaPolicy> lst1 = Arrays.asList(makePolicy(policy1, type));
        List<ToscaPolicy> lst2 = Arrays.asList(makePolicy(policy2, type2));
        List<ToscaPolicy> lst3 = Arrays.asList(makePolicy(policy3, type));
        when(dao.getFilteredPolicyList(any())).thenReturn(lst1, lst2, lst3);

        // load it
        notifier = new MyNotifier(publisher);

        // should have only loaded each policy once
        verify(dao, times(3)).getFilteredPolicyList(any());

        ArgumentCaptor<PolicyPdpNotificationData> captor = ArgumentCaptor.forClass(PolicyPdpNotificationData.class);

        // should have added each policy
