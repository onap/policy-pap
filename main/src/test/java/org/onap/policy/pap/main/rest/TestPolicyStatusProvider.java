...
public class TestPolicyStatusProvider extends ProviderSuper {
    ...
    public void testGetPolicyStatusOneMatch() throws PfModelException {
        ...
        final Collection<PolicyStatus> actual = prov.getByRegex(pattern);
        final PolicyStatus[] objects = actual.toArray(PolicyStatus[]::new);
        final String actualName = objects[0].getPolicy().getName();
    ...
    public void testGetPolicyStatusNoValidRegex() throws PfModelException {
        ...
        Mockito.doReturn(null).when(spy).getStatus(ArgumentMatchers.any(ToscaConceptIdentifierOptVersion.class));

        spy.getByRegex(pattern);

        Mockito.verify(spy, times(1)).getStatus(ArgumentMatchers.any(ToscaConceptIdentifierOptVersion.class));
    }
