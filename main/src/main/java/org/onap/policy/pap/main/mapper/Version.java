...
public class Version implements Comparable<Version> {
     ...
     * @param key the key containing the version, in textual form
     ...
     *         form)
     */
    public static Version makeVersion(String type, PfConceptKey key) {
        Matcher matcher = VERSION_PAT.matcher(key.getVersion());
        if (!matcher.matches()) {
            logger.info("invalid version for {} {}: {}", type, key.getName(), key.getVersion());
            return null;
        }
        ...
        try {
            ...
            logger.info("invalid version for {}: {}", type, key.getVersion(), e);
            return null;
        }
    }
