...
public class PdpGroupDeploymentControllerV1 extends PapRestControllerV1 {
    ...
    @POST
    @Path("pdps")
    @ApiOperation(value = "Deploy or update PDP Group", notes = "Returns PDP deployments",
        response = PdpGroupDeploymentResponse.class,
        tags = {"Deploy PDP Group"},
    ...
    public Response deploy(@HeaderParam(REQUEST_ID_NAME) @ApiParam(REQUEST_ID_PARAM_DESCRIPTION) UUID requestId,
        ...
        resp.setStatus(PdpResponseStatus.SUCCESS);
        resp.setPdps(new LinkedList<>());
