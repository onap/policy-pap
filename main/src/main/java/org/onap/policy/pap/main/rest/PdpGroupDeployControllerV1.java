...
public class PdpGroupDeployControllerV1 extends PapRestControllerV1 {
    ...
    private Response doOperation(UUID requestId, String errmsg, RunnableWithPfEx runnable) {
        try {
            runnable.run();
            return addLoggingHeaders(addVersionControlHeaders(Response.status(Status.ACCEPTED)), requestId).entity(
                new PdpGroupDeployResponse("Use the deployment status url to fetch the latest deployment status.",
