...
public class PapExceptionHandler {
    ...
    /**
     * Handle PfModelException and PfModelRuntimeException.
     *
    ...
    public ResponseEntity<ErrorResponse> pfModelExceptionHandler(PfModelException exp, WebRequest req) {
        ...
        return PapRestControllerV1.addLoggingHeaders(
            PapRestControllerV1.addVersionControlHeaders(
                ResponseEntity.status(exp.getErrorResponse().getResponseCode().getStatusCode())),
            requestId != null ? UUID.fromString(requestId) : null).body(exp.getErrorResponse());
    }
