...
public class PdpGroupDeployProvider {
    ...
    private void processPolicy(SessionData data, PolicyIdentOptVersion desiredPolicy)
        ...
        if (desiredPolicy.getName() == null) {
            throw new PolicyPapRuntimeException("null policy name");
        }
    ...
    /**
     * Creates a policy type identifier from a policy.
     *
     * @param policy source policy
     * @return a policy type identifier
     */
    private PolicyTypeIdent makeType(ToscaPolicy policy) {
        PolicyTypeIdent ident = new PolicyTypeIdent();

        ident.setName(policy.getType());
        ident.setVersion(policy.getTypeVersion());
     ...
     */
    private PdpUpdate makeUpdate(PdpGroup group, PdpSubGroup subgroup, Pdp pdpInstance) {
    ...
    protected static class SessionData {
        ...
        public List<ToscaPolicy> getPolicies(PolicyIdentOptVersion desiredPolicy) throws PfModelException {
            return dao.getPolicies(desiredPolicy.getName(), desiredPolicy.getVersion());
        }

        public ToscaPolicy getPolicyMaxVersion(String name) throws PfModelException {
            return dao.getPolicyMaxVersion(name);
        }

        public PdpGroup getPdpGroupMaxVersion(String name) throws PfModelException {
            return dao.getPdpGroupMaxVersion(name);
        }

        public List<PdpGroup> getActivePdpGroupsByPolicy(String policyType, String policyTypeVersion)
                        throws PfModelException {
            return dao.getActivePdpGroupsByPolicy(policyType, policyTypeVersion);
        }
