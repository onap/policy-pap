...
public class PdpGroupDeployProvider {
    ...
    private void upgradeGroup(SessionData data, ToscaPolicy policy, PdpGroup oldGroup) throws PfModelException {
        ...
        for (PdpSubGroup subgroup : newGroup.getPdpSubgroups()) {
            ...
            // load the subgroup's policies - needed when creating update messages
            data.loadPolicies(subgroup);
    ...
    protected static class SessionData {
        ...
        /**
         * Loads the policies for a particular subgroup into the cache.
         *
         * @param subgroup subgroup whose policies are to be loaded
         * @throws PolicyPapRuntimeException if an error occurs
         */
        public void loadPolicies(PdpSubGroup subgroup) {

            for (ToscaPolicyIdentifier ident : subgroup.getPolicies()) {

                policyMap.computeIfAbsent(ident, key -> {
                    try {
        ...
        /**
         * Gets the policy, referenced by an identifier, from the cache.
         *
        ...
        public ToscaPolicy getPolicy(ToscaPolicyIdentifier ident) {
            ToscaPolicy policy = policyMap.get(ident);
            if (policy == null) {
                throw new PolicyPapRuntimeException("policy not loaded: " + ident);
            }
