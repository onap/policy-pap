...
public class PdpGroupDeployProvider {
    ...
    private void processPolicy(SessionData data, PolicyIdentOptVersion desiredPolicy)
        ...
        Collection<PdpGroup> groups = getGroup(data, policy.getType().getName(), policy.getType().getVersion());
        if (groups.isEmpty()) {
    ...
    private ToscaPolicy getPolicy(SessionData data, PolicyIdentOptVersion desiredPolicy) throws PfModelException {
        ...
        for (ToscaPolicy policy : data.getPolicies(desiredPolicy)) {
            Version vers = Version.makeVersion("ToscaPolicy", policy.getKey().getName(), policy.getKey().getVersion());
     ...
     */
    private Collection<PdpGroup> getGroup(SessionData data, String policyType, String policyTypeVersion)
        ...
        for (PdpGroup group : data.getActivePdpGroupsByPolicy(policyType, policyTypeVersion)) {
            Version vers = Version.makeVersion("PdpGroup", group.getKey().getName(), group.getKey().getVersion());
    ...
    private Version makeNewVersion(SessionData data, PdpGroup group) throws PfModelException {
        PdpGroup group2 = data.getPdpGroupMaxVersion(group.getKey().getName());
