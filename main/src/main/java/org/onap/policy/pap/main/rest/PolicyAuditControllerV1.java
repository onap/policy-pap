...
public class PolicyAuditControllerV1 extends PapRestControllerV1 {
    ...
    @ApiOperation(value = "Queries audit information for all the policies",
        notes = "Queries audit information for all the policies, "
            + "returning audit information for all the policies in database",
    ...
    public Response getAllAuditRecords(
        ...
        try {
            return addLoggingHeaders(addVersionControlHeaders(Response.status(Response.Status.OK)), requestId).entity(
                            provider.getAuditRecords(recordCount, (fromDate == null ? null : fromDate.toInstant()),
                                            (toDate == null ? null : toDate.toInstant())))
                            .build();
    ...
    public Response getAuditRecordsByGroup(
        ...
        try {
            return makeOkOrNotFoundResponse(requestId,
                            provider.getAuditRecords(recordCount, (fromDate == null ? null : fromDate.toInstant()),
                                            (toDate == null ? null : toDate.toInstant()), pdpGroupName));
    ...
    public Response getAuditRecordsOfPolicy(
        ...
        try {
            return makeOkOrNotFoundResponse(requestId,
                            provider.getAuditRecords(recordCount, (fromDate == null ? null : fromDate.toInstant()),
                                            (toDate == null ? null : toDate.toInstant()), pdpGroupName,
                                            new ToscaConceptIdentifier(policyName, policyVersion)));
