...
public class PdpStatisticsListener implements TypedMessageListener<PdpStatus> {
    ...
    private void handleStatistics(final PdpStatus message, final PolicyModelsProvider databaseProvider)
        ...
        boolean pdpFound = databaseProvider.getFilteredPdpGroups(filter).stream()
                        ...
                        .filter(subgrp -> subgrp.getPdpType().equals(message.getPdpType()))
                        .flatMap(subgrp -> subgrp.getPdpInstances().stream())
                        .anyMatch(pdp -> pdp.getInstanceId().equals(message.getName()));
        if (pdpFound) {
            databaseProvider.createPdpStatistics(List.of(message.getStatistics()));
            LOGGER.debug("Created PdpStatistics in DB for {}", message.getName());
        }
    }
    ...
    private boolean validStatistics(final PdpStatus message) {
        ...
        return new EqualsBuilder()
            ...
            .append(message.getStatistics().getPolicyDeployCount() >= 0, true)
            .append(message.getStatistics().getPolicyDeployFailCount() >= 0, true)
            .append(message.getStatistics().getPolicyDeploySuccessCount() >= 0, true)
            .append(message.getStatistics().getPolicyExecutedCount() >= 0, true)
            .append(message.getStatistics().getPolicyExecutedFailCount() >= 0, true)
            .append(message.getStatistics().getPolicyExecutedSuccessCount() >= 0, true)
            .isEquals();
        // @formatter:on
    }
}
