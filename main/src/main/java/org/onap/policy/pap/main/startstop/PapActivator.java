...
public class PapActivator extends ServiceManagerContainer {
    ...
    public PapActivator(final PapParameterGroup papParameterGroup, Properties topicProperties) {
        ...
        AtomicReference<PapDaoFactory> papDaoFactory = new AtomicReference<>();
        ...
        addAction("PAP DAO Factory",
            () -> {
                papDaoFactory.set(new PseudoDaoFactory());
            },
            () -> papDaoFactory.get().close());

        addAction("PAP DAO Factory",
            () -> Registry.register(PapConstants.REG_PAP_DAO_FACTORY, papDaoFactory.get()),
    ...
    // TODO use parameters to create a real factory and then register that instead of this
    private static class PseudoDaoFactory implements PapDaoFactory {
