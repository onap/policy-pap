...
public abstract class MultiPdpStatusListener implements TypedMessageListener<PdpStatus> {
    ...
    public final void onTopicEvent(CommInfrastructure infra, String topic, PdpStatus message) {
        try {
            handleEvent(infra, topic, message);
     ...
     */
    protected abstract void handleEvent(CommInfrastructure infra, String topic, PdpStatus message);

    /**
     * Extracts the ID from the message.
     *
     * @param message message from which the ID should be extracted
     * @return the ID, or {@code null} if it cannot be extracted
     */
    protected abstract String extractId(PdpStatus message);
}
