...
public class PdpStatusMessageHandler {
    ...
    private void handlePdpHeartbeat(final PdpStatus message, final PolicyModelsProvider databaseProvider)
        ...
        final PdpGroups pdpGroups = databaseProvider.getPdpGroups("");
        for (final PdpGroup pdpGroup : pdpGroups.getGroups()) {
            if (pdpGroup.getName().equals(message.getPdpGroup())) {
                pdpInstanceFound = findPdpSubGroup(message, pdpGroup);
            }
        }
        if (!pdpInstanceFound) {
    ...
    private boolean findPdpSubGroup(final PdpStatus message, final PdpGroup pdpGroup) {
        boolean pdpInstanceFound = false;
        for (final PdpSubGroup subGroup : pdpGroup.getPdpSubgroups()) {
            if (message.getPdpSubgroup().equals(subGroup.getPdpType())) {
                pdpInstanceFound = findPdpInstance(message, pdpGroup, subGroup);
            }
        }
        return pdpInstanceFound;
    }

    private boolean findPdpInstance(final PdpStatus message, final PdpGroup pdpGroup, final PdpSubGroup subGroup) {
        boolean pdpInstanceFound = false;
