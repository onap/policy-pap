...
public class PdpStatusMessageHandler {
    ...
    private boolean findAndUpdatePdpGroup(final PdpStatus message, final PolicyModelsProvider databaseProvider)
        ...
        final Map<Integer, PdpGroup> selectedPdpGroups = new TreeMap<>();
        final List<PdpGroup> pdpGroups = databaseProvider.getFilteredPdpGroups(filter);
        emptyPdpGroup = selectPdpGroupsForRegistration(message, selectedPdpGroups, emptyPdpGroup, pdpGroups);
        if (emptyPdpGroup != null) {
            pdpGroupFound = registerPdp(message, databaseProvider, pdpGroupFound, emptyPdpGroup);
        } else if (selectedPdpGroups.size() > 0) {
            final PdpGroup finalizedPdpGroup =
                    selectedPdpGroups.get(selectedPdpGroups.keySet().toArray()[selectedPdpGroups.keySet().size() - 1]);

            pdpGroupFound = registerPdp(message, databaseProvider, pdpGroupFound, finalizedPdpGroup);
        }
        return pdpGroupFound;
    }

    private PdpGroup selectPdpGroupsForRegistration(final PdpStatus message,
            final Map<Integer, PdpGroup> selectedPdpGroups, PdpGroup emptyPdpGroup, final List<PdpGroup> pdpGroups) {
    ...
    private boolean registerPdp(final PdpStatus message, final PolicyModelsProvider databaseProvider,
            boolean pdpGroupFound, final PdpGroup finalizedPdpGroup) throws PfModelException {
