...
public class PapRestApiController implements PapRestApi {
    ...
    public ResponseEntity<Void> reportPdpStatusPut(@ApiParam(
            value = "The parameters containing the PDP status information") @Valid 
            @RequestBody PdpStatusParameters pdpStatusParameters) {
        try {
            papService.handlePdpStatusReport(pdpStatusParameters);
        } catch (NexusRestWrapperException | IOException | ApiException exception) {
            logger.error("Error handling PDP status report", exception);
