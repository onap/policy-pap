...
function setEngineSummaryData(data, timeStamp, policyDeployCount, policyDeploySuccessCount, policyDeployFailCount, policyExecutedCount, policyExecutedSuccessCount, policyExecutedFailCount) {
    this._setEngineSummaryData(timeStamp, getUptimeOfOldestEngine(data), getSumOfPolicyExecutions(data),
            getAvgPolicyDuration(data), policyDeployCount, policyDeploySuccessCount, policyDeployFailCount, policyExecutedCount, policyExecutedSuccessCount, policyExecutedFailCount);
}
 ...
 * necessary
 */
function _setEngineSummaryData(timestamp, upTime, policyExecutions, avgPolicyDuration, policyDeployCount, policyDeploySuccessCount, policyDeployFailCount, policyExecutedCount, policyExecutedSuccessCount, policyExecutedFailCount) {

    var tableId = config.engineSummary.tableId;
    var headers = config.engineSummary.headers.map(function(a) {
        return a.id;
    });
    var data = [ timestamp, upTime, policyDeployCount, policyDeploySuccessCount, policyDeployFailCount, policyExecutions>0? policyExecutions:policyExecutedCount, policyExecutedSuccessCount, policyExecutedFailCount ];
    ...
    if (avgPolicyDuration.length) {
        ...
        papUtils_removeElement(chartConfig.parent);
    }
}
