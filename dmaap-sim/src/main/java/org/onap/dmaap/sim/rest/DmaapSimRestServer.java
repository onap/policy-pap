...
import org.onap.dmaap.sim.parameters.RestServerParameters;
...
public class DmaapSimRestServer implements Startable {
    ...
    public boolean start() {
        try {
            ...
            for (final HttpServletServer server : servers) {
                if (server.isAaf()) {
                    server.addFilterClass(null, DmaapSimAafFilter.class.getCanonicalName());
                }
                server.start();
            }
    ...
    private Properties getServerProperties() {
        ...
        props.setProperty(svcpfx + PolicyEndPointProperties.PROPERTY_HTTP_REST_CLASSES_SUFFIX,
                        String.join(",", DmaapSimRestControllerV1.class.getCanonicalName()));
